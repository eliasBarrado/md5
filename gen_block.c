...
/*		if(i > 2e10){
	    ...
	    // Q[2]  = ^^^^ 110. ..0^ ^^^0 1..^ 1... ^10. .00. Changed
	    Q [2]  = frandom() & 0x71de7799 | 0x0c008840 |  (QM0 ^ 0x80000000) & 0x80000000;
	    Q1[2]  = Q[2] ^ 0x82000020;
	    ...
	    Q [4]  = frandom() & 0x01c0e000 | 0x3a040010 | Q[3] & 0x80000601;
	    Q1[4]  = Q[4] ^ 0x8600003e;

	    //  Q[5]  = !100 10.. ..10 1111 ...0 1110 0101 0000   
	    // ∆Q[5]  = 0000 0000 0-++ ++++ ++++ ++++ ++00 0000
	    Q [5]  = frandom() & 0x03c0e000 | 0x482f0e50 | (Q[4] ^ 0x80000000) & 0x80000000;
	    Q1[5]  = Q[5] ^ 0x80001fc1;

	    //  Q[6]  = ^..0 010. 1.10 ..10 11.0 1100 0101 0110
	    // ∆Q[6]  = +000 0000 +000 0000 0000 0000 0-00 0000
	    Q [6]  = frandom() & 0x614c2000 | 0x04a2cc56 | Q[5] & 0x80000000;
	    Q1[6]  = Q[6] ^ 0x80330000;
	    ...
	    //  Q[7]  = !..1 011^ 1.00 ..01 10.1 1110 00.. ...1 
	    // ∆Q[7]  = ++++ ++-- -000 0000 0000 -+++ ++-+ ++++
	    Q [7] = frandom() & 0x604c203e | 0x16819e01 | Q[6] & 0x01000000 | (Q[6] ^ 0x80000000) & 0x80000000;
	    Q1[7] = Q[7] ^ 0x980003c0;
	    ...
	    // ∆Q[13] = +000 00+- 0000 0000 0000 0000 0000 0000
	    Q [13] = frandom() & 0x00701f77 | 0x3f0fe008 | (Q[12] ^ 0x80000000) & 0x80000000;
	    ...
	    // ∆Q[14] = +000 0000 0000 0000 0000 0000 0000 0000
	    Q [14] = frandom() & 0x00701f77 | 0x408be088 | Q[13] & 0x80000000;
	    Q1[14] = Q[14] ^ 0x80000000;
	    ...
	    //  Q[16] = 0.10 .... .... ..!. .... .... .... .... 
	    // ∆Q[16] = +0-0 0000 0000 0000 0000 0000 0000 0000
	    Q [16] = frandom() & 0x4ffdffff | 0x20000000 | (Q[15] ^ 0x00020000) & 0x00020000;
	    Q1[16] = Q[16] ^ 0xa0000000;
	    ...
	    for( int k = 0; k < 128; k++){
	    	...
	    	// Q[1]  = !... 010. ..1. ...0 .... 0... .10. .... 
	    	Q [1]  = frandom() & 0x01c0e71f | 0x04200040 | Q[2] & 0xf01e1080;
	    	Q1[1]  = Q[1] ^ 0x82000000;
		...
		if( QM3_1 != QM3 || QM2_1 != QM2 || QM1_1 != QM1 || QM0_1 != QM0){
			QM3 = IV1;
		  	QM2 = IV4;
		  	QM1 = IV3;
		  	QM0 = IV2;
...
int main (void){
	...
	printf("\nStarting first block computation with initial values:\n");
	printf("IV1 = %x ", IV1 );
	printf("IV2 = %x ", IV2 );
	printf("IV3 = %x ", IV3 );
	printf("IV4 = %x \n", IV4 );
	...
	printf("\nBlock 1 found in %lf seconds\n\n", cpu_time_used  );

	printf("Intermediate hash values:\n");
	...
	printf("IV1_1 = %x ", IV1_1 );
	printf("IV2_1 = %x ", IV2_1 );
	printf("IV3_1 = %x ", IV3_1 );
	printf("IV4_1 = %x\n", IV4_1 );

	start_t = clock();

	printf("\nStarting second block computation\n");
	block2();
