...
int block(void){
	...
	while(1){
	    ...
	    // Q1[i] = Q[i] for i = 1,2,3,4 hence m1[j] = m[j] for j = 0,1,2,3
	    m1[ 0] = m[0];
	    m1[ 1] = m[1];
	    m1[ 2] = m[2];
	    m1[ 3] = m[3];
	...
	if(!MESSAGEMOD){
		...
		for(k = 0; k < 128; k++){
			...
			Q1[1] = RL( m1[ 0] + F(QM0  , QM1  , QM2  ) + QM3 + 0xd76aa478, 7 ) + QM0;  
	...
	if(MESSAGEMOD){	    
	    ...
	    if( bit(Q[19],18) ){
	    	...
	    	Q [10] = RL( m [ 9] + F(Q [ 9], Q [ 8], Q [ 7]) + Q [ 6] + 0x8b44f7af, 12) + Q [ 9];
	    	Q1[10] = RL( m1[ 9] + F(Q1[ 9], Q1[ 8], Q1[ 7]) + Q1[ 6] + 0x8b44f7af, 12) + Q1[ 9];

	    	m [10] = RR(Q [11] - Q [10], 17) - F(Q [10], Q [ 9], Q [ 8]) - Q [ 7] - 0xffff5bb1;
	    	m1[10] = RR(Q1[11] - Q1[10], 17) - F(Q1[10], Q1[ 9], Q1[ 8]) - Q1[ 7] - 0xffff5bb1; 

	    	m [11] = m[11] - 0x00000004;
	    	m1[11] = m[11] + 0x00008000;
	    ...
	    if( bit(Q[19],32) ){
	    	...
	    	Q [16] = Q[16] - 0x00010000 + 0x00400000;
	    	Q1[16] = Q[16] ^ 0xa0000000;
